    ...
    <div class="bg-gray-100 rounded-3xl px-4 py-3 mx-4 border border-gray-200 flex flex-col">
        ...
        <div class="flex mt-3">
            ...
            <a-tooltip placement="top">
                <!-- Tooltip 提示文字 -->
                <template #title>
                  <span>{{ props.loading ? '停止生成' : '请输入你的问题' }}</span>
                </template>

                <!-- 发送按钮 -->
                <button v-if="!props.loading" class="flex items-center justify-center bg-[#4d6bfe] rounded-full w-8 h-8 border border-[#4d6bfe] hover:bg-[#3b5bef] transition-colors
                        disabled:opacity-50
                ...
                <!-- 停止按钮 -->
                <button v-else class="flex items-center justify-center bg-red-500 rounded-full w-8 h-8 border border-red-500 hover:bg-red-600 transition-colors"
